...
st.markdown('<h2 class="main-title">KPI METRICS VIEW FOR LOSS RATIO WITH ACTUAL CLAIM AMOUNT</h2>', unsafe_allow_html=True)
...
if not df.empty:
    ...
    total_pro = (df_proactiv["Total"].sum())/scale
    total_health = (df_health["Total"].sum())/scale
